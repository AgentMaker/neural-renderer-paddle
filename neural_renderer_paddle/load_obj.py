...
import neural_renderer_paddle.cuda.load_textures as load_textures_cuda
...
def load_textures(filename_obj, filename_mtl, texture_size, texture_wrapping='REPEAT', use_bilinear=True):
    ...
    faces = paddle.to_tensor(faces).cuda()

    colors, texture_filenames = load_mtl(filename_mtl)

    textures = paddle.zeros([faces.shape[0], texture_size, texture_size, texture_size, 3], dtype=paddle.float32) + 0.5
    textures = textures.cuda()

    #
    for material_name, color in colors.items():
        color = paddle.to_tensor(color).cuda()
        for i, material_name_f in enumerate(material_names):
            if material_name == material_name_f:
                textures[i, :, :, :, :] = color[None, None, None, :].astype(textures.dtype)
    ...
    for material_name, filename_texture in texture_filenames.items():
        ...
        image = image[::-1, :, :]
        image = paddle.to_tensor(image.copy()).cuda()
        is_update = (np.array(material_names) == material_name).astype(np.int32)
        is_update = paddle.to_tensor(is_update).cuda()
...
def load_obj(filename_obj, normalization=True, texture_size=4, load_texture=False,
    ...
    vertices = paddle.to_tensor(np.vstack(vertices).astype(np.float32)).cuda()
    ...
    for line in lines:
        ...
        if line.split()[0] == 'f':
            ...
            for i in range(nv - 2):
                ...
                faces.append((v0, v1, v2))
    faces = paddle.to_tensor(np.vstack(faces).astype(np.int32)).cuda() - 1
    ...
    if normalization:
        ...
        vertices /= paddle.abs(vertices).max()
        vertices *= 2
