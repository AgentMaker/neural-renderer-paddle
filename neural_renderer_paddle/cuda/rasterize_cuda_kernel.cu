...
__global__ void forward_face_index_map_cuda_kernel_1(
        const data_t* __restrict__ faces,
        data_t* __restrict__ faces_inv,
        int batch_size,
        int num_faces,
        int image_size) {
    ...
    const data_t* face = &faces[i * 9];
    data_t* face_inv_g = &faces_inv[i * 9];
...
__global__ void forward_face_index_map_cuda_kernel_2(
        ...
        data_t* __restrict__ weight_map,
        data_t* __restrict__ depth_map,
        data_t* __restrict__ face_inv_map,
        int batch_size,
        int num_faces,
        int image_size,
        data_t near,
    ...
    const int yi = pn / is;
    const int xi = pn % is;
    const data_t yp = (2. * yi + 1 - is) / is;
    const data_t xp = (2. * xi + 1 - is) / is;
    
    const data_t* face = &faces[bn * nf * 9] - 9;
    data_t* face_inv = &faces_inv[bn * nf * 9] - 9;
    data_t depth_min = far;
    int face_index_min = -1;
    data_t weight_min[3];
    data_t face_inv_min[9];
    for (int fn = 0; fn < nf; fn++) {
        ...
        const data_t zp = 1. / (w[0] / face[2] + w[1] / face[5] + w[2] / face[8]);
        if (zp <= near || far <= zp) {
            continue;
        }
...
__global__ void forward_texture_sampling_cuda_kernel(
		const data_t* faces,
		const data_t* textures,
		const int32_t* face_index_map,
		const data_t* weight_map,
		const data_t* depth_map,
		data_t* rgb_map,
		int32_t* sampling_index_map,
        data_t* sampling_weight_map,
        size_t batch_size,
        int num_faces,
        int image_size,
        int texture_size,
    ...
    if (face_index >= 0) {
        ...
        */
        ...
        const data_t* face = &faces[(bn * nf + face_index) * 9];
        const data_t* texture = &textures[(bn * nf + face_index) * ts * ts * ts * 3];
        data_t* pixel = &rgb_map[i * 3];
        const data_t* weight = &weight_map[i * 3];
        const data_t depth = depth_map[i];
        int32_t* sampling_indices = &sampling_index_map[i * 8];
        data_t* sampling_weights = &sampling_weight_map[i * 8];
    
        /* get texture index (float) */
        data_t texture_index_float[3];
        for (int k = 0; k < 3; k++) { data_t tif = weight[k] * (ts - 1) * (depth / (face[3 * k + 2]));
            tif = max(tif, 0.);
    ...
    for (int edge_num = 0; edge_num < 3; edge_num++) {
        ...
        for (int axis = 0; axis < 2; axis++) {
            ...
            for (int d0 = d0_from; d0 <= d0_to; d0++) {
                /* get cross point */
                int d1_in, d1_out;
                const data_t d1_cross = (p[1][1] - p[0][1]) / (p[1][0] - p[0][0]) * (d0 - p[0][0]) + p[0][1];
                if (0 < direction)
                ...
                if (is_in_fn) {
                    ...
                    data_t* alpha_map_p;
                    data_t* grad_alpha_map_p;
                    data_t* rgb_map_p;
                    data_t* grad_rgb_map_p;
                    ...
                    for (int d1 = d1_from; d1 <= d1_to; d1++) {
                        ...
                        if (diff_grad <= 0)
                            continue;
                        if (p[1][0] != d0) {
                            data_t dist = (p[1][0] - p[0][0]) / (p[1][0] - d0) * (d1 - d1_cross) * 2. / is;
                            dist = (0 < dist) ? dist + eps : dist - eps;
                            grad_face[pi[0] * 3 + (1 - axis)] -= diff_grad / dist;
                        }
                        if (p[0][0] != d0) {
                            data_t dist = (p[1][0] - p[0][0]) / (d0 - p[0][0]) * (d1 - d1_cross) * 2. / is;
                            dist = (0 < dist) ? dist + eps : dist - eps;
...
__global__ void backward_textures_cuda_kernel(
    ...
    if (0 <= face_index) {
        ...
        data_t* grad_texture = &grad_textures[(bn * nf + face_index) * ts * ts * ts * 3];
        data_t* sampling_weight_map_p = &sampling_weight_map[i * 8];
        int* sampling_index_map_p = &sampling_index_map[i * 8];
        for (int pn = 0; pn < 8; pn++) {
            data_t w = *sampling_weight_map_p++;
            int isc = *sampling_index_map_p++;
            data_t* grad_texture_p = &grad_texture[isc * 3];
            data_t* grad_rgb_map_p = &grad_rgb_map[i * 3];
            for (int k = 0; k < 3; k++)
...
__global__ void backward_depth_map_cuda_kernel(
        ...
        const data_t*  depth_map,
        const int32_t* face_index_map,
        const data_t* face_inv_map,
        const data_t* weight_map,
        data_t*  grad_depth_map,
        data_t*  grad_faces,
        size_t batch_size,
        size_t num_faces,
        int image_size) {
    ...
    if (0 <= fn) {
        ...
        const int is = image_size;
        const int bn = i / (is * is);
        const data_t* face = &faces[(bn * nf + fn) * 9];
        const data_t depth = depth_map[i];
        const data_t depth2 = depth * depth;
        const data_t* face_inv = &face_inv_map[i * 9];
        const data_t* weight = &weight_map[i * 3];
        const data_t grad_depth = grad_depth_map[i];
        data_t* grad_face = &grad_faces[(bn * nf + fn) * 9];
    
        /* derivative wrt z */
        for (int k = 0; k < 3; k++) {
            const data_t z_k = face[3 * k + 2];
...
std::vector<paddle::Tensor> forward_face_index_map_cuda(
    ...
    const auto batch_size = faces.shape()[0];
    const auto num_faces = faces.shape()[1];
    const int threads = 512;
    const dim3 blocks_1 ((batch_size * num_faces - 1) / threads +1);

    PD_DISPATCH_FLOATING_TYPES(faces.type(), "forward_face_index_map_cuda_1", ([&] {
        forward_face_index_map_cuda_kernel_1<data_t><<<blocks_1, threads>>>(
            faces.data<data_t>(),
            const_cast<data_t*>(faces_inv.data<data_t>()),
            batch_size,
            num_faces,
            image_size);
    }));
    ...
    if (err != cudaSuccess) 
        PD_THROW("Error in forward_face_index_map_1: %s\n", cudaGetErrorString(err));

    const dim3 blocks_2 ((batch_size * image_size * image_size - 1) / threads +1);
    
    PD_DISPATCH_FLOATING_TYPES(faces.type(), "forward_face_index_map_cuda_2", ([&] {
        forward_face_index_map_cuda_kernel_2<data_t><<<blocks_2, threads>>>(
            faces.data<data_t>(),
            faces_inv.data<data_t>(),
            const_cast<int32_t*>(face_index_map.data<int32_t>()),
            const_cast<data_t*>(weight_map.data<data_t>()),
            const_cast<data_t*>(depth_map.data<data_t>()),
            const_cast<data_t*>(face_inv_map.data<data_t>()),
            (int) batch_size,
            (int) num_faces,
            (int) image_size,
            (data_t) near,
    ...
    if (err != cudaSuccess) 
        PD_THROW("Error in forward_face_index_map_2: %s\n", cudaGetErrorString(err));
...
std::vector<paddle::Tensor> forward_texture_sampling_cuda(
    ...
    const auto batch_size = faces.shape()[0];
    const auto num_faces = faces.shape()[1];
    const auto texture_size = textures.shape()[2];
    const int threads = 512;
    const dim3 blocks ((batch_size * image_size * image_size - 1) / threads + 1);

    PD_DISPATCH_FLOATING_TYPES(faces.type(), "forward_texture_sampling_cuda", ([&] {
        forward_texture_sampling_cuda_kernel<data_t><<<blocks, threads>>>(
            faces.data<data_t>(),
            textures.data<data_t>(),
            face_index_map.data<int32_t>(),
            weight_map.data<data_t>(),
            depth_map.data<data_t>(),
            const_cast<data_t*>(rgb_map.data<data_t>()),
            const_cast<int32_t*>(sampling_index_map.data<int32_t>()),
            const_cast<data_t*>(sampling_weight_map.data<data_t>()),
            batch_size,
            num_faces,
            image_size,
            texture_size,
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) 
        PD_THROW("Error in forward_texture_sampling: %s\n", cudaGetErrorString(err));
...
paddle::Tensor backward_pixel_map_cuda(
    ...
    PD_DISPATCH_FLOATING_TYPES(faces.type(), "backward_pixel_map_cuda", ([&] {
        backward_pixel_map_cuda_kernel<data_t><<<blocks, threads>>>(
            faces.data<data_t>(),
            face_index_map.data<int32_t>(),
            rgb_map.data<data_t>(),
            alpha_map.data<data_t>(),
            grad_rgb_map.data<data_t>(),
            grad_alpha_map.data<data_t>(),
            const_cast<data_t*>(grad_faces.data<data_t>()),
            batch_size,
            num_faces,
            image_size,
            (data_t) eps,
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) 
        PD_THROW("Error in backward_pixel_map: %s\n", cudaGetErrorString(err));
...
paddle::Tensor backward_textures_cuda(
    ...
    const auto batch_size = face_index_map.shape()[0];
    const auto image_size = face_index_map.shape()[1];
    const auto texture_size = grad_textures.shape()[2];
    const int threads = 512;
    const dim3 blocks ((batch_size * image_size * image_size - 1) / threads + 1);

    PD_DISPATCH_FLOATING_TYPES(sampling_weight_map.type(), "backward_textures_cuda", ([&] {
      backward_textures_cuda_kernel<data_t><<<blocks, threads>>>(
          face_index_map.data<int32_t>(),
          sampling_weight_map.data<data_t>(),
          sampling_index_map.data<int32_t>(),
          grad_rgb_map.data<data_t>(),
          const_cast<data_t*>(grad_textures.data<data_t>()),
          batch_size,
          num_faces,
          image_size,
          texture_size);
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) 
        PD_THROW("Error in backward_textures: %s\n", cudaGetErrorString(err));
...
paddle::Tensor backward_depth_map_cuda(
    ...
    PD_DISPATCH_FLOATING_TYPES(faces.type(), "backward_depth_map_cuda", ([&] {
        backward_depth_map_cuda_kernel<data_t><<<blocks, threads>>>(
            faces.data<data_t>(),
            depth_map.data<data_t>(),
            face_index_map.data<int32_t>(),
            face_inv_map.data<data_t>(),
            weight_map.data<data_t>(),
            grad_depth_map.data<data_t>(),
            const_cast<data_t*>(grad_faces.data<data_t>()),
            batch_size,
            num_faces,
            image_size);
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) 
        PD_THROW("Error in backward_depth_map: %s\n", cudaGetErrorString(err));
