...
class TestRasterizeSilhouettes(unittest.TestCase):
    ...
    def test_backward_case1(self):
        ...
        vertices = paddle.to_tensor(np.array(vertices, np.float32)).cuda()
        faces = paddle.to_tensor(np.array(faces, np.int32)).cuda()
        grad_ref = paddle.to_tensor(np.array(grad_ref, np.float32)).cuda()
