...
class TestRasterize(unittest.TestCase):
    ...
    def test_backward_case1(self):
        ...
        vertices = paddle.to_tensor(np.array(vertices, dtype=np.float32)).cuda()
        faces = paddle.to_tensor(np.array(faces, dtype=np.int32)).cuda()
        textures = paddle.ones([faces.shape[0], 4, 4, 4, 3], dtype=paddle.float32).cuda()
        grad_ref = paddle.to_tensor(np.array(grad_ref, dtype=np.float32)).cuda()
        ...
        images, _, _ = renderer(vertices, faces, textures)
        images = paddle.mean(images, axis=1)
        loss = paddle.sum(paddle.abs(images[:, pyi, pxi] - 1))
        loss.backward()

        assert(paddle.allclose(vertices.grad, grad_ref, rtol=1e-2))
    ...
    def test_backward_case2(self):
        ...
        images, _, _ = renderer(vertices, faces, textures)
        images = paddle.mean(images, axis=1)
        loss = paddle.sum(paddle.abs(images[:, pyi, pxi]))
        loss.backward()
