...
class TestCore(unittest.TestCase):
    def test_tetrahedron(self):
        ...
        assert (paddle.allclose(paddle.to_tensor(vertices_ref).cuda(), vertices))
        assert (paddle.allclose(paddle.to_tensor(faces_ref).cuda().astype('float32'), faces.astype('float32')))
        vertices, faces = nr.load_obj(obj_file, True)
        assert (paddle.allclose(paddle.to_tensor(vertices_ref).cuda() * 2 - 1.0, vertices))
        assert (paddle.allclose(paddle.to_tensor(faces_ref).cuda().astype('float32'), faces.astype('float32')))
    ...
    def test_texture(self):
        ...
        images = images.transpose([0,2,3,1]).detach().cpu().numpy()
